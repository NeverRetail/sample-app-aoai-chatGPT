...
def generateFilterString(userToken):
    ...
    if userGroups:
        ...
        return (
            f"{AZURE_SEARCH_PERMITTED_GROUPS_COLUMN}/any(g:search.in(g, '{group_ids}'))"
        )
        ...
        "dataSources": [
            {
                ...
                "parameters": {
                    ...
                    if len(AZURE_OPENAI_EMBEDDING_NAME) == 0
                    else "",
...
def conversation_without_data(request_body):
    ...
    if not SHOULD_STREAM:
        ...
        return Response(
            stream_without_data(response, history_metadata),
            mimetype="text/event-stream",
        )
...
def generate_title(conversation_messages):
    ...
    try:
        ...
        completion = openai.ChatCompletion.create(
            engine=AZURE_OPENAI_MODEL, messages=messages, temperature=1, max_tokens=64
        )
